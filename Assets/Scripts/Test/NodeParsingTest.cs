...
public class NodeParsingTest: MonoBehaviour {
	...
	private void Update() {
		var tempPos = _a.transform.position;
		var tempDirection = _b.transform.position - tempPos;
		Debug.DrawRay(pos, direction, Color.red);
		Debug.DrawRay(pos, t * direction, Color.blue);
		
		if (pos == tempPos && direction == tempDirection)
			return;

		pos = tempPos;
		direction = tempDirection;
		var result = _root.IsThrough(new() { Origin = pos, Length = direction }, out t);
		Debug.Log(result);
	}
	...
	private void OnDrawGizmos() {
		...
		foreach (var box in GetBox(_root, _layerCnt)) {
