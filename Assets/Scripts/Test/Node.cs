...
public class Node {
	...
	public bool IsThrough(Ray pRay, out float t) {
		if (Triangle != null)
			return IsThroughTriangle(pRay, out t);
		var result = CheckChild(Left, out t);
		if (!result)
			result = CheckChild(Right, out t);
		return result;

		bool CheckChild(Node pChild, out float t) {
			if (pChild.Triangle == null) {
				t = -1;
				return pChild.IsThroughAABB(pRay) && pChild.IsThrough(pRay, out t);
			}
			return pChild.IsThroughTriangle(pRay, out t);
		}
	}
	...
	private bool IsThroughTriangle(Ray pRay, out float t) {
		if (Triangle == null) {
			t = -1;
			return false;
		}
